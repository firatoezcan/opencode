export * as AISDKAdapter from "./aisdk-adapter"
...
export interface Options {
  ...
  readonly networkError?: (message: string) => unknown
}
  ...
  switch (event.type) {
    ...
    case "finish-step":
      if (event.rawFinishReason === "network_error")
        return Effect.fail(
          options.networkError?.("Provider finish_reason: network_error") ??
      ...
      return Effect.sync(() => {
        ...
        return [
          LLMEvent.toolError({
            id: event.toolCallId,
            name,
            message: (options.errorMessage ?? message)(event.error),
            error: event.error,
            providerMetadata: providerMetadata(event.providerMetadata),
          }),
        ]
