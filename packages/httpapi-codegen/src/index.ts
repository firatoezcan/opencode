...
export function emitPromise(
  contract: Contract,
  options?: {
    readonly runtimeOutputs?: ReadonlySet<string>
  },
): Output {
  ...
  const runtime = renderPromiseRuntime(groups, options?.runtimeOutputs)
  return {
    ...
    files: [
      ...
      ...(runtime === undefined ? [] : [{ path: "schema.ts", content: runtime }]),
      {
        path: "index.ts",
        content:
...
function renderPromiseRuntime(groups: ReadonlyArray<Group>, outputs: ReadonlySet<string> | undefined) {
  if (outputs === undefined || outputs.size === 0) return undefined
  const endpoints = groups.flatMap((group) => group.endpoints)
  const byName = new Map(endpoints.map((endpoint) => [`${endpoint.group}.${endpoint.operation.name}`, endpoint]))
  const missing = Array.from(outputs).find((name) => !byName.has(name))
  if (missing !== undefined) throw new GenerationError({ reason: `Missing Promise runtime output: ${missing}` })
  const slots = endpoints.flatMap((endpoint) => {
    const operation = `${endpoint.group}.${endpoint.operation.name}`
    if (!outputs.has(operation)) return []
    return [
      {
        name: `${promiseTypePrefix(endpoint.group, endpoint.operation.name)}OutputSchema`,
        schema: promiseOutputSchema(endpoint),
        exported: true,
      },
    ]
  })
  const imports = slots.map((slot) => slot.name.replace(/Schema$/, ""))
  const decoders = slots.map((slot) => {
    const output = slot.name.replace(/Schema$/, "")
    return `export const decode${output} = (value: unknown): ${output} => Schema.decodeUnknownSync(${slot.name})(value)`
  })
  return `// Generated by @opencode-ai/httpapi-codegen. Do not edit.\nimport { Schema } from "effect"\nimport type { ${imports.join(", ")} } from "./types"\n\n${renderSchemas(slots)}\n\n${decoders.join("\n\n")}\n`
}

function promiseOutputSchema(endpoint: Endpoint) {
  const success = endpoint.successes[0]
  if (isStreamSchema(success)) {
    if (success._tag !== "StreamSse" || success.sseMode !== "data") {
      throw new GenerationError({
        reason: `Unsupported Promise runtime output: ${endpoint.group}.${endpoint.operation.name}`,
      })
    }
    return streamEncodedDataSchema(success)
  }
  if (HttpApiSchema.isNoContent(success.ast)) {
    throw new GenerationError({
      reason: `Unsupported Promise runtime output: ${endpoint.group}.${endpoint.operation.name}`,
    })
  }
  const encoded = Schema.toEncoded(success)
  if (!endpoint.unwrapData) return encoded
  if (!SchemaAST.isObjects(encoded.ast)) {
    throw new GenerationError({
      reason: `Invalid Promise runtime output: ${endpoint.group}.${endpoint.operation.name}`,
    })
  }
  const data = encoded.ast.propertySignatures.find((field) => field.name === "data")?.type
  if (data === undefined) {
    throw new GenerationError({
      reason: `Invalid Promise runtime output: ${endpoint.group}.${endpoint.operation.name}`,
    })
...
function renderSchemas(slots: ReadonlyArray<Slot>) {
  ...
  const declarations = slots.map((slot, index) => {
    ...
    if (tagged === undefined) {
      return `${slot.exported ? "export " : ""}const ${slot.name} = ${document.codes[index].runtime}`
    }
